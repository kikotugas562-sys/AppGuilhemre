...
async function getPageHtml(url, allowReader=true){
  let html = await fetchWithTimeout(url, 2200);
  if(html) return html;
  if(allowReader){
    try{
      const ru = "https://r.jina.ai/" + url;
      const txt = await fetchWithTimeout(ru, 2800);
...
async function searchCompany(input){
  ...
  maxPages = Math.max(3, Math.min(maxPages, 20));
...
exports.handler = async (event) => {
  ...
  try{
    ...
    catch { return json(400, {ok:false, error:"Pedido inválido: JSON mal formado."}); }
